...
test("CT-003 Não deve logar com formatação do email incorreta", async ({ page }) => {
  await page.login.visit();
  await page.login.loginForm("admin.zombieplus.com", "pwd123");  
...
test("CT-007 Não deve logar se email e senha não foram preenchida", async ({ page }) => {
  await page.login.visit();
  await page.login.loginForm("", "");
  await page.login.alertHasText([
    "Campo obrigatório",
    "Campo obrigatório"
    ]);
});
