...
test("CT-002 Não deve cadastrar um lead com email já cadastrado", async ({
  page,
  request,
}) => {
  const leadName = faker.person.fullName();
...
test('CT-004 Não deve cadastrar se "nome" não for preenchido', async ({
  page,
}) => {
  await page.leads.visit();
  await page.leads.openModal();
...
test('CT-005 Não deve cadastrar se "email" não for preenchido', async ({
  page,
}) => {
  await page.leads.visit();
  await page.leads.openModal();
...
test('CT-006 Não deve cadastrar se os campos" não forem preenchidos', async ({
  page,
}) => {
  await page.leads.visit();
  await page.leads.openModal();
